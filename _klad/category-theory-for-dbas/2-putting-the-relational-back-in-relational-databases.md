...
And as noted earlier, sorting strings shows how $x \le y$ and $y \le x$ does not have to imply that $x=y$. This is precisely how SQL Server uses collation, sorting order, to check strings for equivalence. In fact these pair are interesting in their own right, because they represent something that is *like* equality, without being equal. These pairs form what is called an equivalence relation.
...
The relevant database concept is a table with a primary key, which is precisely the constraint needed to turn a relation into a function, namely that a value is guaranteed to only exist in one row (pair). It is also common to keep track of the possible outputs of each function. If $X$ is the set of inputs of a function and its outputs are in $Y$ then this is denoted $f: X \to Y$. In databases this is done by specifying a foreign key, which ensure that the output falls within a column of some other table.

The nice thing about functions is that they compose nicely. If there are two functions $f: X \to Y$ and $g:Y \to Z$ then they can be combined into a new relation $g \circ f$ which maps $x$ to $g(f(x))$. This new relation is also a function. Making this work seamlessly is precisely why it's so useful to keep track of the possible inputs and outputs.

However just functions are not the only type of relation that can be composed, functions just do it in a way that behaves better. The more general way to compose relations is in fact a well known concept in relational databases.
...
Hopefully this has given a bit of insight into the properties a relation may have and how these do or don't correspond ot things you can do in a database.

However as stated in the beginning of the article this is just the binary relations. Converting those back to a database you'd end up with a database containing tables with only 2 columns (to the delight of advocates for RDF triples[^trip]), in practice this is not what databases look like. Does this mean that knowledge of binary relations is not useful? Well obviously not, it just means we need to decompose those $n$-ary relations into binary relations.

Is anything lost by decomposing $n$-ary relations into binary relations? Well, it is entirely possible some properties are hidden by only considering binary relations, but mathematics is not quite ready to deal with $n$-ary relations for $n>2$ in any way that makes sense. So for all intents and purposes understanding binary relations is the best way to understand relational databases.

A well designed database also makes it easy to find useful binary relations. Take for instance a product table, it will contain various properties of each product such as its size, colour, possibly price etc. And usually it will also contain a product ID, a meaningless number. However if it is meaningless then why include it? Since by design the product ID will uniquely refer to a single row which means each column of the table can be turned into a function from the product ID to the value in that column for that row. 

This could be done with a whole lot of 2-column tables, but it is convenient to include all properties a product has in the same table. That way it is easier to know where to look, get a full list of products and use relations between properties of the product. By allowing a column to have empty values it is even possible to include properties which are only defined for *some* products. In this way the product ID becomes a representative for the product itself since properties of the product are turned into functions of the product ID.

Now if binary relations are so powerful then why are people looking for alternatives to SQL databases? There are a couple of reasons, some of which have little to do with relations, but one of the reasons is that SQL doesn't support all things possible to do with a relation equally well. Transitive closures in particular have varying and often lacking support in relational databases that use SQL.

Transitive closures are a way to find the smallest transitive relation (or a pre-order) containing some other relation as a subset (compare figure 1 and 2 for an example). Since a transitive relation can grow quite large it's often not practical to store it in full so first-class support for transitive closures is important. Consider how keeping track of a hierarchy is usually done by only storing the next-highest level, or how equivalence relations may sometimes need to be built from a partial list of equivalent objects. This also relates to the notion of clustering, which will be the subject of the next post in this series.
