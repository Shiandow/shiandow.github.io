...
A well designed database also makes it easy to find useful binary relations. Take for instance a product table, it will contain various properties of each product such as its size, colour, possibly price etc. And usually it will also contain a product ID, a meaningless number. However if it is meaningless then why include it? Since by design the product ID will uniquely refer to a single row it allows each column of the table to be turned into a function from the product ID to the value in that column for that row. 

This could be done with a whole lot of 2-column tables, but it is convenient to include all properties a product has in the same table. That way it is easier to know where to look, to get a full list of products and use relations between properties of the product. By allowing a column to have empty values it is even possible to include properties which are only defined for *some* products. In this way the product ID becomes a representative for the product itself since properties of the product are turned into functions of the product ID.

Now if binary relations are so powerful then why are people looking for alternatives to SQL databases? There are a couple of reasons, some of which have little to do with relations, but one of the reasons is that SQL doesn't support all things possible to do with a relation equally well. Transitive closure in particular has varying and often lacking support in relational databases that use SQL.[^break]
